...
└── Generation (llm) - llm-response
    ...
    └── costDetails: { input: 0.002, output: 0.001, total: 0.003 }
```
...
- `generation`: LLM 调用（使用 Langfuse Generation API，支持 usage/cost 详情）
- `span`: 工具调用
...
```typescript
// 使用 Prompt 标签获取指定版本
const prompt = await this.promptManager.fetchPrompt(
  'planner-decision',
  'production'
);

// 获取编译后的 Prompt（变量替换）
const compiled = await this.promptManager.getCompiledPrompt(
  'planner-decision',
  {
...
当设置 `LANGFUSE_PROMPT_LABEL` 时，系统会尝试从 Langfuse 平台获取指定标签的 Prompt 版本。如果获取失败，会自动降级到本地注册的 Prompt。
...
> **注意**：LLM 调用现在使用 Generation API 记录，相比 Span 可以更精确地追踪 Token 使用量（usageDetails）和成本（costDetails）。
