...
| --------- | ------- | -------- | ----------------------------------------- |
| file_path | string  | Yes      | Path to the file to create                |
| overwrite | boolean | No       | Overwrite if file exists (default: false) |

### Returns

String indicating success with the file path, or error message.
...
- `FILE_ALREADY_EXISTS`: File already exists (when overwrite is false)
- `IS_DIRECTORY`: Path appears to be a directory (ends with / or \\)
